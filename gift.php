...
*/
  ...
  $col = 0;
    ...
    $p_pic = '<a href="' . $specials['products_id'] . '-p-'.str_replace('#','',str_replace(' ','_',$specials['products_name'])) . '-'.str_replace('#','',str_replace(' ','_',$product['products_description'] )). '.html">' . tep_image(DIR_WS_PWS_IMAGE . $product['products_image'], $product['products_name'], SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT) . '</a>';

    $p_desc = ''.substr(strip_tags($product['products_description']), 0, MAX_DESCR_1).' ...';

    $p_name = '<a href="' . $specials['products_id'] . '-p-'.str_replace('#','',str_replace(' ','_',$specials['products_name'])) . '-'.str_replace('#','',str_replace(' ','_',$product['products_description'] )). '.html">' .display_product_name($product['products_name'] , $product).'</a>';
