...
*/
  ...
  if (empty($infos['total'])) {
    ...
    //$query = "DELETE FROM " . TABLE_SPECIALS . " where category = '".date('D')."'";
    //tep_db_query($query);
...
<?php
  $specials_query_raw = "select p.products_quantity, p.products_id, pd.products_name, p.products_price, p.products_tax_class_id, p.products_image, s.specials_new_products_price from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd, " . TABLE_SPECIALS . " s where s.expires_date >= '".date('Y-m-d 00:00:00')."' and s.category = '".date('D')."' and p.products_status = '1' and s.products_id = p.products_id and p.products_id = pd.products_id and pd.language_id = '" . (int)$languages_id . "' and s.status = '1' order by s.specials_date_added DESC";
...
// ----------
	...
	$p_pic = '<a href="' . $specials['products_id'] . '-p-'.str_replace(' ','_',$specials['products_name']). '-'.str_replace(' ','_',$product['products_description'] ) . '.html">' . tep_image(DIR_WS_PWS_IMAGE . $specials['products_image'], $specials['products_name'], SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT) . '</a>';

	$p_name = '<a href="' . $specials['products_id'] . '-p-'.str_replace(' ','_',$specials['products_name']). '-'.str_replace(' ','_',$product['products_description'] ) . '.html">' .display_product_name($specials['products_name'], $product)  .'</a>';
                    ...
                    <tr>
                        <td style="border:1px solid #FFFFFF; border-width:0 5px 0 0;">
                            <table cellpadding="0" cellspacing="0" border="0">
                                ...
                                <tr>
                                    <td style="height:47px "'.($has_discount ? ' colspan="2"' : '').'><a href="' . tep_href_link('product_info.php?products_id='.$p_id) . '">'.tep_image_button("button_details.gif").'</a><br />'.tep_draw_separator('spacer.gif', '1', '3').'</td>
                                </tr>
                            </table>
                        </td>
                    </tr>
