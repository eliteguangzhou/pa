...
<?php
 ...
 $product_query = tep_db_query("select products_description, products_id, Brand, Gender, Gamme, Prix_achat, Note, Annee, Item_size from " . TABLE_PRODUCTS_DESCRIPTION . " where products_id = '" . (int)$products_new['products_id'] . "' and language_id = '" . (int)$languages_id . "'");
      $product = tep_db_fetch_array($product_query);
		...
		$p_pic = '<a href="' . $products_new['products_id'] . '-p-'.str_replace(' ','_',$products_new['products_name']) . '-'.str_replace(' ','_',$product['products_description'] ). '.html">' . tep_image(DIR_WS_PWS_IMAGE . $products_new['products_image'], $products_new['products_name'], SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT) . '</a>';
		$p_name = '<a href="' . $products_new['products_id'] . '-p-'.str_replace(' ','_',$products_new['products_name']) . '-'.str_replace(' ','_',$product['products_description'] ). '.html">' .display_product_name($products_new['products_name'], $product) . '</a>';
