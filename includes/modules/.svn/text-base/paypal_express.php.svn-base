...
// class methods
    ...
    function before_process() {
      global $cart, $order, $sendto, $ppe_token, $ppe_payerid, $HTTP_POST_VARS, $comments, $easy_discount, $nb_products_discount, $check_server;
      ...
      $params = array('USER' => MODULE_PAYMENT_PAYPAL_EXPRESS_API_USERNAME,
                      ...
                      'LANDINGPAGE' => 'Billing',
                      'LOCALECODE' => strtoupper($check_server),
      ...
      foreach ($order->products as $index => $info) {
	       ...
	       $params['L_AMT'.$index]     = $this->format_raw($info['final_price']);
	       $params['L_NUMBER'.$index]  = $info['model'];
	       $params['L_QTY'.$index]     = $info['qty'];
           $params['ITEMAMT'] += $this->format_raw($info['final_price']) * $info['qty'];
           $params['AMT'] += $this->format_raw($info['final_price']) * $info['qty'];
           $lasti = $index;
	  }
