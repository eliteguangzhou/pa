      ...
      for ($col=0, $n=sizeof($column_list); $col<$n; $col++) {
        ...
        switch ($column_list[$col]) {
          ...
          case 'PRODUCT_LIST_NAME':
            ...
            $p_name = $lc_text = '<a href="' . $listing['products_id'] . '-p-'.str_replace(' ','_',$listing['products_name']) .'-'.str_replace(' ','_',$product['products_description'] ) . '.html">' . display_product_name($listing['products_name'], $product) . '</a>';
            } else {
            $p_name = $lc_text = '<a href="' . $listing['products_id'] . '-p-'.str_replace(' ','_',$listing['products_name']) .'-'.str_replace(' ','_',$product['products_description'] ) . '.html">' . display_product_name($listing['products_name'], $product) . '</a>';
            }
          ...
          case 'PRODUCT_LIST_IMAGE':
            $lc_align = 'center';
            if (isset($HTTP_GET_VARS['manufacturers_id'])) {
              $p_pic = '<a href="' . $listing['products_id'] . '-p-'.str_replace(' ','_',$listing['products_name']) . '-'.str_replace(' ','_',$product['products_description'] ) .'.html">' . tep_image(DIR_WS_PWS_IMAGE . $listing['products_image'], $listing['products_name'], SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT) . '</a>';
            } else {
              $p_pic = '<a href="' . $listing['products_id'] . '-p-'.str_replace(' ','_',$listing['products_name']) . '-'.str_replace(' ','_',$product['products_description'] ) .'.html">' . tep_image(DIR_WS_PWS_IMAGE . $listing['products_image'], $listing['products_name'], SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT) . '</a>';
            }
            break;
