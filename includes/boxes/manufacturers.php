...
*/
  ...
  $manufacturers_query = tep_db_query("select DISTINCT(manufacturers_name) as manufacturers_name, m.manufacturers_id from " . TABLE_MANUFACTURERS . " m LEFT JOIN " . TABLE_PRODUCTS . " p on (p.manufacturers_id = m.manufacturers_id) WHERE !ISNULL(p.products_id) GROUP BY manufacturers_name order by manufacturers_name");
...
//         $manufacturers_list .= '<a href="' . tep_href_link(FILENAME_DEFAULT, 'manufacturers_id=' . $manufacturers['manufacturers_id']) . '">' . $manufacturers_name . '</a><br>';
	$manufacturers_list .= '<a href="/'. $manufacturers['manufacturers_id'].'-'.$manufacturers_name .'">' . $manufacturers_name . '</a><br>';
      }
